...
public class dll{
	...
	void addF(int data){
		...
		new_node.prev=null;//assign prev of new node null
		new_node.next=head;//and next of new node as head
		if (head!=null){  //ie if head exists,make the prev of head as newnode
			head.prev=new_node;}
		head=new_node;}//and finally the new node as head
	...
	public static void main(String a[]){
			...
			do{
				...
				switch(ch){
					...
					if (temp.prev==null){// ie if we want to delete the head
						list.head=temp.next; //then temp.next becomes the head
					}
					else{
						temp.prev.next=temp.next; else 
					}	
					break;		
					case 5:list.display();
					break;
